...
Server::Server(Consistency consistency,
               ...
               std::vector<Transport *> transports, const TrueTime &tt,
               bool debug_stats, uint64_t network_latency_window,
               uint8_t sent_redundancy, uint64_t loop_queue_interval_us)
  ...
  for (int redundancy_idx = 0; redundancy_idx < sent_redundancy_;
       redundancy_idx++) {
    transport_->Register(this, shard_configs_[redundancy_idx], shard_idx_,
                         replica_idx_, redundancy_idx);
    //            transport_->Register(this, shard_configs_[i], shard_idx_,
    //            replica_idx_);
  }

  for (int i = 0; i < shard_config_.g; i++) {
    shard_clients_.push_back(
        new ShardClient(shard_configs_, transports, server_id_, i));
  }
  Debug("jenndebug loop queue interval %lu us", loop_queue_interval_us_);

  //        replica_client_ =
  //                new ReplicaClient(replica_config_, transport_, server_id_,
  //                shard_idx_);
...
void Server::NotifySlowPathROs(const std::unordered_set<uint64_t> &ros,
                               uint64_t rw_transaction_id, bool is_commit,
...
void Server::SendROSlowPath(uint64_t ro_transaction_id,
                            uint64_t rw_transaction_id, bool is_commit,
                            const Timestamp &commit_ts) {
  ...
  // Debug("[%lu] Sending slow path reply for %lu", ro_transaction_id,
  // rw_transaction_id);
  ...
  if (transactions_.GetROTransactionState(ro_transaction_id) != SLOW_PATH) {
    ...
    // Debug("s: %d",
    // static_cast<int>(transactions_.GetROTransactionState(ro_transaction_id)));
...
void Server::HandleROCommit(const TransportAddress &remote,
  ...
  min_prepare_timestamp_ =
      std::max(min_prepare_timestamp_, commit_ts);  // TODO: is this correct?
  // ^ Bro how would i know

  TransactionState s =
      transactions_.StartRO(transaction_id, keys, min_ts, commit_ts);
  if (s == PREPARE_WAIT) {
    Debug("[%lu] Waiting for prepared transactions", transaction_id);
    //        auto reply = new PendingROCommitReply(client_id, client_req_id,
    //        remote.clone()); reply->n_slow_path_replies =
    //        transactions_.GetRONumberSkipped(transaction_id);
  ...
  //    if (consistency_ == RSS &&
  //    transactions_.GetRONumberSkipped(transaction_id) > 0) {
  //        const std::vector<PreparedTransaction> skipped_prepares =
  //        transactions_.GetROSkippedRWTransactions(transaction_id);
  //
  //        // Add for slow replies
  //        auto reply = new PendingROCommitReply(client_id, client_req_id,
  //        remote.clone()); reply->n_slow_path_replies =
  //        skipped_prepares.size(); pending_ro_commit_replies_[transaction_id]
  //        = reply;
  //
  //        for (auto &pt : skipped_prepares) {
  //            //Debug("[%lu] replying with skipped prepare: %lu",
  //            transaction_id, pt.transaction_id());
  //            proto::PreparedTransactionMessage *ptm =
  //            ro_commit_reply_.add_prepares(); pt.serialize(ptm);
  //        }
  //
...
void Server::ContinueROCommit(uint64_t transaction_id) {
  ...
  if (consistency_ == RSS &&
    ...
    for (auto &pt : skipped_prepares) {
      // Debug("[%lu] replying with skipped prepare: %lu", transaction_id,
      // pt.transaction_id());
...
void Server::HandleRWCommitCoordinator(/*uint64_t transaction_id, const std::string& key, std::string& value,
  ...
  while (!this->queue_.empty() && should_check_queue) {
    ...
    if (pendingOp.execute_time() <= now) {
      ...
      if (pendingOpType == PUT) {
        Debug("jenndebug [%lu] executing PUT %s, %s, %s", transaction_id,
              key.c_str(), val.c_str(), commit_ts.to_string().c_str());
        ...
        transactions_.read_results(transaction_id)[key] =
            std::pair<std::string, uint64_t>(std::get<1>(value),
                                             std::get<2>(value));

        Debug("jenndebug [%lu] executing GET %s, commit_ts %s", transaction_id,
              key.c_str(), commit_ts.to_string().c_str());
      }
      ...
      if (0 == transactions_.still_pending_ops(transaction_id)) {
        if (!transactions_.is_inconsistent(transaction_id)) {
          Debug("jenndebug [%lu][replica %d] sending success", transaction_id,
                replica_idx_);
          SendRWCommmitCoordinatorReplyOK(
              transaction_id, commit_ts, nonblock_ts,
  ...
  if (!queue_.empty()) {
    cancel_timer_fd_ = transport_->TimerMicro(
        loop_queue_interval_us_,
        std::bind(&Server::HandleRWCommitCoordinator, this));
  }
}

void Server::EnqueueOps(const TransportAddress &remote,
  ...
  const RequestID requestId(client_id, client_req_id, &remote);
  {
    ...
    //            lock(multi_sent_reqs_recvd_yet_mutex_);
    if (multi_sent_reqs_recvd_yet_.find(requestId) !=
        multi_sent_reqs_recvd_yet_.end()) {
      // already seen this request, don't need to process it again
      multi_sent_reqs_recvd_yet_[requestId]++;
      Debug("jenndebug [%lu] redundancy %d", transaction_id,
            multi_sent_reqs_recvd_yet_[requestId]);
  ...
  //        Debug("jenndebug [%lu] Coordinator for transaction",
  //        transaction_id);
  ...
  TransactionState s = transactions_.StartCoordinatorPrepare(
      ...
      nonblock_ts);
  if (s == PREPARING) {
    //            Debug("jenndebug [%lu] Coordinator preparing",
    //            transaction_id);

    // for (LockAcquireResult ar = locks_.AcquireLocks(transaction_id,
    // transaction); ar.status != LockStatus::ACQUIRED; ar =
    // locks_.AcquireLocks(transaction_id, transaction)) {}
    ...
    //            Debug("jenndebug [%lu] transactions.is_inconsistent() %s",
    //            transaction_id,
    //                  transactions_.is_inconsistent(transaction_id) ? "true" :
    //                  "false");
    //            Debug("jenndebug [%lu] write_set size %lu", transaction_id,
    //            transaction.getWriteSet().size());
    for (auto &write : transaction.getWriteSet()) {
      const std::chrono::microseconds network_latency_window =
          store_.get_network_latency_window(write.first);

      PendingOp pendingOp(PUT, write.first, write.second, commit_ts,
                          transaction_id, nonblock_ts, network_latency_window);

      //                if (pendingOp.execute_time() < now_tt.mid()) {
      //                    stats_.Increment("missed_latency_window_" +
      //                    std::to_string(client_id)); stats_.Add("missed_by_"
      //                    + std::to_string(client_id) + "_us",
      //                               now_tt.mid() - pendingOp.execute_time());
      //
      //                    TimestampID lastRead(0, 0);
      //                    if (store_.lastRead(pendingOp.key(), lastRead) &&
      //                        lastRead > TimestampID(pendingOp.commit_ts(),
      //                        pendingOp.transaction_id())) {
      //                        transactions_.mark_inconsistent(transaction_id,
      //                        true); stats_.Increment("inconsistent_");
      //                    }
      //                    Debug("jennbdebug [%lu] missed latency_window by a
      //                    bit, transactions missed window",
      //                          transaction_id);
      //
      //                    // let the write through, even though we're not on
      //                    time. The odds that we're not on time to
      ...
      Debug("jenndebug [%lu] enqueued PUT %s, %s", transaction_id,
            write.first.c_str(), write.second.c_str());
      //
    ...
    for (auto &key : transaction.getPendingReadSet()) {
      const std::chrono::microseconds network_latency_window =
          store_.get_network_latency_window(key);
      PendingOp pendingOp(GET, key, "", commit_ts, transaction_id, nonblock_ts,
    ...
    transactions_.still_pending_ops(transaction_id) +=
        transaction.getPendingReadSet().size();
    //            this->transaction_still_pending_ops_[transaction_id] +=
    //            transaction.getPendingReadSet().size();

    uint64_t wait_until_us = latest_execution_time > now_tt.mid()
                                 ? latest_execution_time - now_tt.mid()
                                 : 0;
    //        Debug("jenndebug latest_execution_time [%lu], tt_.Now().mid()
    //        [%lu]", latest_execution_time, tt_.Now().mid());

    //            if (!set_off_timer_yet_) {
    //                /*int fd = */ transport_->TimerMicro(wait_until_us,
    //                                                     std::bind(&Server::HandleRWCommitCoordinator,
    //                                                     this));
    ...
    if (transactions_.is_inconsistent(transaction_id)) {
      Debug("jenndebug [%lu][replica %d] send fail, req_id %lu", transaction_id,
            replica_idx_, client_req_id);
      SendRWCommmitCoordinatorReplyFail(remote, client_id, client_req_id);
    }
    //            Debug("[%lu] Coordinator for wait_until_us %lu",
    //            transaction_id, wait_until_us);
    //
    // LockReleaseResult rr = locks_.ReleaseLocks(transaction_id, transaction);
  ...
  //     LockReleaseResult rr = locks_.ReleaseLocks(transaction_id,
  //     transaction);
  //
  //     SendRWCommmitCoordinatorReplyFail(remote, client_id, client_req_id);
  //
  ...
  //
  //     auto reply = new PendingRWCommitCoordinatorReply(client_id,
  //     client_req_id, remote.clone());
  //     pending_rw_commit_c_replies_[transaction_id] = reply;
  //
...
void Server::ContinueCoordinatorPrepare(uint64_t transaction_id) {
  ...
  if (s == PREPARING) {
    ...
    if (ar.status == LockStatus::ACQUIRED) {
      ...
      //                        commit_ts,
      //                        std::bind(&Server::CommitCoordinatorCallback,
      //                        this,
      //                                  transaction_id,
      //                                  std::placeholders::_1),
      //                        []() {}, COMMIT_TIMEOUT);
...
void Server::SendRWCommmitCoordinatorReplyOK(
  ...
  for (const auto &kv : reads) {
    ...
    //            Debug("jenndebug [%lu] rreply %s, %s",transaction_id,
    //            rreply->key().c_str(), rreply->val().c_str());
  }
...
void Server::HandleRWCommitParticipant(const TransportAddress &remote,
  ...
  if (s == PREPARING) {
    ...
    if (ar.status == LockStatus::ACQUIRED) {
      ...
      //                        std::bind(&Server::PrepareCallback, this,
      //                        transaction_id,
      //                                  std::placeholders::_1,
      //                                  std::placeholders::_2),
...
void Server::PrepareOKCallback(uint64_t transaction_id, int status,
                               Timestamp commit_ts) {
  Debug("[%lu] Received PREPARE_OK callback: %d %d", transaction_id, shard_idx_,
        status);
...
void Server::CoordinatorCommitTransaction(uint64_t transaction_id,
  ...
  // NotifyPendingROs(fr.notify_ros);
  // NotifySlowPathROs(fr.notify_slow_path_ros, transaction_id, true,
  // commit_ts);
}
...
void Server::ReplicaUpcall(opnum_t opnum, const string &op, string &response) {
  ...
  if (request.op() == strongstore::proto::Request::PREPARE) {
    ...
    //
    //     s = transactions_.StartParticipantPrepare(transaction_id,
    //     coordinator, transaction, nonblock_ts); ASSERT(s == PREPARING);
    //
    //     LockAcquireResult ar = locks_.AcquireLocks(transaction_id,
    //     transaction); ASSERT(ar.status == LockStatus::ACQUIRED);
    //
    //     transactions_.SetParticipantPrepareTimestamp(transaction_id,
    //     prepare_ts);
    //
    ...
    if (request.has_prepare()) {  // Coordinator commit
      ...
      if (transactions_.GetRWTransactionState(transaction_id) != COMMITTING) {
        ...
        // for (int p : participants) {
        //     if (p != coordinator) {
        //         s = transactions_.CoordinatorReceivePrepareOK(transaction_id,
        //         p, commit_ts, nonblock_ts);
        //     }
        // }
        // ASSERT(s == PREPARING);
      ...
      transport_->TimerMicro(
          commit_wait_us, std::bind(&Server::CoordinatorCommitTransaction, this,
                                    transaction_id, commit_ts));
