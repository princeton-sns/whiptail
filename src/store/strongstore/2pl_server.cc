...
TwoPLServer::TwoPLServer(Consistency consistency,
                         ...
                         bool debug_stats)
    : Server(consistency, shard_config, replica_config, server_id, shard_idx,
             replica_idx, transport, tt, debug_stats),
      tt_{tt}, transactions_{shard_idx, consistency, tt_},
      shard_config_{shard_config}, replica_config_{replica_config},
      transport_{transport}, server_id_{server_id}, min_prepare_timestamp_{},
      shard_idx_{shard_idx}, replica_idx_{replica_idx},
      consistency_{consistency}, debug_stats_{debug_stats} {}
...
Stats &TwoPLServer::GetStats() {
  Stats &s = transactions_.GetStats();
  stats_.Merge(s);
  return stats_;
}

void TwoPLServer::ReceiveMessage(const TransportAddress &remote,
...
void TwoPLServer::HandleGet(const TransportAddress &remote, proto::Get &msg) {
...
void TwoPLServer::ContinueGet(uint64_t transaction_id) {
...
const Timestamp TwoPLServer::GetPrepareTimestamp(uint64_t client_id) {
  uint64_t ts =
...
void TwoPLServer::WoundPendingRWs(uint64_t transaction_id,
...
void TwoPLServer::NotifyPendingRWs(uint64_t transaction_id,
                                   const std::unordered_set<uint64_t> &rws) {
...
void TwoPLServer::NotifyPendingROs(const std::unordered_set<uint64_t> &ros) {
  for (uint64_t waiting_ro : ros) {
    ContinueROCommit(waiting_ro);
  }
}

void TwoPLServer::NotifySlowPathROs(const std::unordered_set<uint64_t> &ros,
...
void TwoPLServer::SendROSlowPath(uint64_t ro_transaction_id,
                                 uint64_t rw_transaction_id, bool is_commit,
                                 const Timestamp &commit_ts) {
...
void TwoPLServer::HandleROCommit(const TransportAddress &remote,
                                 proto::ROCommit &msg) {
  uint64_t client_id = msg.rid().client_id();
  uint64_t client_req_id = msg.rid().client_req_id();
  uint64_t transaction_id = msg.transaction_id();
...
void TwoPLServer::ContinueROCommit(uint64_t transaction_id) {
...
void TwoPLServer::HandleRWCommitCoordinator(const TransportAddress &remote,
  ...
  if (s == PREPARING) {
    ...
    for (LockAcquireResult ar =
             locks_.AcquireLocks(transaction_id, transaction);
         ar.status != LockStatus::ACQUIRED;
         ar = locks_.AcquireLocks(transaction_id, transaction)) {
    }
...
void TwoPLServer::ContinueCoordinatorPrepare(uint64_t transaction_id) {
  ...
  if (s == PREPARING) {
    ...
    if (ar.status == LockStatus::ACQUIRED) {
      ...
      replica_client_->CoordinatorCommit(
          ...
          std::bind(&TwoPLServer::CommitCoordinatorCallback, this,
                    transaction_id, std::placeholders::_1),
          []() {}, COMMIT_TIMEOUT);
...
void TwoPLServer::SendRWCommmitCoordinatorReplyOK(
    uint64_t transaction_id, const Timestamp &commit_ts,
    const Timestamp &nonblock_ts) {
  Debug("[%lu] Sending RW Commit Coordinator reply");
  auto search = pending_rw_commit_c_replies_.find(transaction_id);
  if (search == pending_rw_commit_c_replies_.end()) {
...
void TwoPLServer::SendRWCommmitCoordinatorReplyFail(
    const TransportAddress &remote, uint64_t client_id,
...
void TwoPLServer::SendPrepareOKRepliesOK(uint64_t transaction_id,
                                         const Timestamp &commit_ts) {
...
void TwoPLServer::SendPrepareOKRepliesFail(PendingPrepareOKReply *reply) {
...
void TwoPLServer::CommitCoordinatorCallback(uint64_t transaction_id,
...
void TwoPLServer::SendRWCommmitParticipantReplyOK(uint64_t transaction_id) {
...
void TwoPLServer::SendRWCommmitParticipantReplyFail(uint64_t transaction_id) {
  auto search = pending_rw_commit_p_replies_.find(transaction_id);
  ASSERT(search != pending_rw_commit_p_replies_.end());
...
void TwoPLServer::SendRWCommmitParticipantReplyFail(
    const TransportAddress &remote, uint64_t client_id,
    uint64_t client_req_id) {
  rw_commit_p_reply_.mutable_rid()->set_client_id(client_id);
  rw_commit_p_reply_.mutable_rid()->set_client_req_id(client_req_id);
...
void TwoPLServer::HandleRWCommitParticipant(const TransportAddress &remote,
  ...
  if (s == PREPARING) {
    ...
    if (ar.status == LockStatus::ACQUIRED) {
      ...
      replica_client_->Prepare(
          ...
          std::bind(&TwoPLServer::PrepareCallback, this, transaction_id,
      ...
      shard_clients_[coordinator]->PrepareAbort(
          ...
          std::bind(&TwoPLServer::PrepareAbortCallback, this, transaction_id,
...
void TwoPLServer::ContinueParticipantPrepare(uint64_t transaction_id) {
  auto search = pending_rw_commit_p_replies_.find(transaction_id);
...
void TwoPLServer::PrepareCallback(uint64_t transaction_id, int status,
                                  Timestamp timestamp) {
  ...
  if (s == PREPARED) {
    ...
    shard_clients_[coordinator]->PrepareOK(
        ...
        std::bind(&TwoPLServer::PrepareOKCallback, this, transaction_id,
                  placeholders::_1, placeholders::_2),
        [](int, Timestamp) {}, PREPARE_TIMEOUT);
...
void TwoPLServer::PrepareOKCallback(uint64_t transaction_id, int status,
                                    Timestamp commit_ts) {
  ...
  if (status == REPLY_OK) {
    ...
    replica_client_->Commit(
        ...
        std::bind(&TwoPLServer::CommitParticipantCallback, this, transaction_id,
                  std::placeholders::_1),
        []() {}, COMMIT_TIMEOUT);
    ...
    replica_client_->Abort(
        transaction_id,
        std::bind(&TwoPLServer::AbortParticipantCallback, this, transaction_id),
        []() {}, ABORT_TIMEOUT);
...
void TwoPLServer::PrepareAbortCallback(uint64_t transaction_id, int status,
                                       Timestamp timestamp) {
  ASSERT(status == REPLY_OK);
...
void TwoPLServer::CommitParticipantCallback(uint64_t transaction_id,
                                            transaction_status_t status) {
  ASSERT(status == REPLY_OK);
...
void TwoPLServer::AbortParticipantCallback(uint64_t transaction_id) {
  Debug("[%lu] Received ABORT participant callback: %d", transaction_id,
        shard_idx_);
}

void TwoPLServer::HandlePrepareOK(const TransportAddress &remote,
                                  proto::PrepareOK &msg) {
  uint64_t client_id = msg.rid().client_id();
  uint64_t client_req_id = msg.rid().client_req_id();
...
void TwoPLServer::HandlePrepareAbort(const TransportAddress &remote,
...
void TwoPLServer::HandleWound(const TransportAddress &remote,
                              proto::Wound &msg) {
  uint64_t transaction_id = msg.transaction_id();
...
void TwoPLServer::HandleAbort(const TransportAddress &remote,
                              proto::Abort &msg) {
  uint64_t transaction_id = msg.transaction_id();
...
void TwoPLServer::SendAbortParticipants(
...
void TwoPLServer::CoordinatorCommitTransaction(uint64_t transaction_id,
                                               const Timestamp commit_ts) {
...
void TwoPLServer::ParticipantCommitTransaction(uint64_t transaction_id,
                                               const Timestamp commit_ts) {
  Debug("[%lu] Commiting", transaction_id);
...
void TwoPLServer::LeaderUpcall(opnum_t opnum, const string &op, bool &replicate,
                               string &response) {
 ...
 */
void TwoPLServer::ReplicaUpcall(opnum_t opnum, const string &op,
                                string &response) {
  ...
  if (request.op() == strongstore::proto::Request::PREPARE) {
    ...
    if (request.has_prepare()) { // Coordinator commit
      ...
      transport_->TimerMicro(
          commit_wait_us, std::bind(&TwoPLServer::CoordinatorCommitTransaction,
...
void TwoPLServer::UnloggedUpcall(const string &op, string &response) {
  NOT_IMPLEMENTED();
}

void TwoPLServer::Load(const string &key, const string &value,
