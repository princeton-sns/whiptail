...
class RssCodebase:
    ...
    def get_client_cmd(self, config, i, k, run, local_exp_directory,
        ...
        if bench_mode == 'open':
            client_command += ' --client_arrival_rate %f' % config[
                'client_arrival_rate']
            client_command += ' --client_think_time %f' % config[
                'client_think_time']
            client_command += ' --client_stay_probability %f' % config[
                'client_stay_probability']
        elif bench_mode == 'closed':
            client_command += ' --mpl=%d' % config['mpl']

        if 'client_switch_probability' in config:
            client_command += ' --client_switch_probability %f' % config[
                'client_switch_probability']
        ...
        if config['replication_protocol'] == 'tapir':
            if 'sync_commit' in config['replication_protocol_settings']:
                client_command += ' --tapir_sync_commit=%s' % (
                    str(config['replication_protocol_settings'][
                            'sync_commit']).lower())
        ...
        if 'message_transport_type' in config['replication_protocol_settings']:
            client_command += ' --trans_protocol %s' % \
                              config['replication_protocol_settings'][
        ...
        if 'client_message_timeout' in config:
            client_command += ' --message_timeout %d' % config[
                'client_message_timeout']
        if 'client_abort_backoff' in config:
            client_command += ' --abort_backoff %d' % config[
                'client_abort_backoff']
        ...
        if 'client_max_attempts' in config:
            client_command += ' --max_attempts %d' % config[
                'client_max_attempts']
        ...
        if config['benchmark_name'] == 'retwis':
            ...
            if 'client_key_selector' in config:
                client_command += ' --key_selector %s' % config[
                    'client_key_selector']
            if config['client_key_selector'] == 'zipf':
                client_command += ' --zipf_coefficient %f' % config[
                    'client_zipf_coefficient']

        if 'client_wrap_command' in config and len(
                config['client_wrap_command']) > 0:
        ...
        if isinstance(config['client_debug_output'], str) or config[
            'client_debug_output']:
            if 'run_locally' in config and config[
                'run_locally'] or 'default_remote_shell' in config and config[
                'default_remote_shell'] == 'bash':
    ...
    def get_replica_cmd(self, config, instance_idx, shard_idx, replica_idx, run,
                        local_exp_directory,
                        remote_exp_directory):
        ...
        if config['replication_protocol'] == 'strong':
            ...
            if 'max_dep_depth' in config['replication_protocol_settings']:
                replica_command += ' --strong_max_dep_depth %d' % \
                                   config['replication_protocol_settings'][
                                       'max_dep_depth']
            if 'unreplicated' in config['replication_protocol_settings']:
        ...
        if config['replication_protocol'] == 'tapir':
            if 'strictly_serializable' in config[
                'replication_protocol_settings']:
                replica_command += ' --tapir_linearizable=%s' % str(
                    config['replication_protocol_settings'][
                        'strictly_serializable']).lower()

        if config['replication_protocol'] == 'morty' or config[
            'replication_protocol'] == 'morty-context':
        ...
        if config['replication_protocol'] == 'indicus' or config[
            'replication_protocol'] == 'pbft' or config[
            'replication_protocol'] == 'hotstuff':
            if 'read_dep' in config['replication_protocol_settings']:
                replica_command += ' --indicus_read_dep %s' % \
                                   config['replication_protocol_settings'][
                                       'read_dep']
            if 'watermark_time_delta' in config[
                'replication_protocol_settings']:
            ...
            if 'signature_type' in config['replication_protocol_settings']:
                replica_command += ' --indicus_key_type %d' % \
                                   config['replication_protocol_settings'][
                                       'signature_type']
            if 'sig_batch' in config['replication_protocol_settings']:
                replica_command += ' --indicus_sig_batch %d' % \
                                   config['replication_protocol_settings'][
                                       'sig_batch']
            ...
            if 'ebatch_size' in config['replication_protocol_settings']:
                replica_command += ' --indicus_esig_batch %d' % \
                                   config['replication_protocol_settings'][
                                       'ebatch_size']
            ...
            if 'multi_threading' in config['replication_protocol_settings']:
                replica_command += ' --indicus_multi_threading=%s' % str(
                    config['replication_protocol_settings'][
                        'multi_threading']).lower()
            if 'batch_verification' in config['replication_protocol_settings']:
                replica_command += ' --indicus_batch_verification=%s' % str(
                    config['replication_protocol_settings'][
                        'batch_verification']).lower()
            if 'batch_verification_size' in config[
                'replication_protocol_settings']:
                replica_command += ' --indicus_batch_verification_size %d' % \
                                   config[
                                       'replication_protocol_settings'][
                                       'batch_verification_size']
        ...
        if config['benchmark_name'] == 'retwis':
            ...
            if 'server_preload_keys' in config:
                replica_command += ' --preload_keys=%s' % str(
                    config['server_preload_keys']).lower()
        elif config['benchmark_name'] == 'tpcc' or config[
            'benchmark_name'] == 'tpcc-sync':
            replica_command += ' --data_file_path %s' % config[
                'tpcc_data_file_path']
            replica_command += ' --tpcc_num_warehouses %d' % config[
                'tpcc_num_warehouses']
        elif config['benchmark_name'] == 'smallbank':
            replica_command += ' --data_file_path %s' % config[
                'smallbank_data_file_path']

        if 'partitioner' in config:
            replica_command += ' --partitioner %s' % config['partitioner']

        if 'server_wrap_command' in config and len(
                config['server_wrap_command']) > 0:
        ...
        if isinstance(config['server_debug_output'], str) or config[
            'server_debug_output']:
            if 'run_locally' in config and config[
                'run_locally'] or 'default_remote_shell' in config and config[
                'default_remote_shell'] == 'bash':
    ...
    def prepare_local_exp_directory(self, config, config_file):
        ...
        elif "server_emulate_wan" in config and not config[
            "server_emulate_wan"]:
            rtts = {}
    ...
    def prepare_remote_server_codebase(self, config, host, local_exp_directory,
