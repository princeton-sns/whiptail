...
class RssCodebase:
  ...
  def get_client_cmd(
    ...
    replica_configs = [
        config['replica_config_format_str'] % (i) for i in range(num_instances)
    ]
    shard_configs = [
        config['shard_config_format_str'] % (i) for i in range(num_instances)
    ]
    ...
    if 'run_locally' in config and config['run_locally']:
      ...
      replica_config_paths = map(
          lambda c: os.path.join(local_exp_directory, c), replica_configs
      )
      shard_config_paths = map(
          lambda c: os.path.join(local_exp_directory, c), shard_configs
      )
      ...
      replica_config_paths = map(
          lambda c: os.path.join(remote_exp_directory, c), replica_configs
      )
      shard_config_paths = map(
          lambda c: os.path.join(remote_exp_directory, c), shard_configs
      )
      network_config_path = os.path.join(
    ...
    truetime_error = (
        config['truetime_error'] if 'truetime_error' in config else 0
    )

    if (
        config['read_percent']
        + config['write_percent']
        + config['mixed_rw_percent']
        != 100
    ):
      raise RuntimeError(
    ...
    client_command = ' '.join([
        ...
        for x in [
            path_to_client_bin,
            '--write_ops_txn',
            config['write_ops_txn'],
            '--read_ops_txn',
            config['read_ops_txn'],
            '--mixed_write_ops_txn',
            config['mixed_write_ops_txn'],
            '--mixed_read_ops_txn',
            config['mixed_read_ops_txn'],
            '--read_percent',
            config['read_percent'],
            '--write_percent',
            config['write_percent'],
    ...
    if bench_mode == 'open':
      client_command += (
          ' --client_arrival_rate %f' % config['client_arrival_rate']
      )
      client_command += ' --client_think_time %f' % config['client_think_time']
      client_command += (
          ' --client_stay_probability %f' % config['client_stay_probability']
      )
    ...
    if 'client_switch_probability' in config:
      client_command += (
          ' --client_switch_probability %f'
          % config['client_switch_probability']
      )
    ...
    if config['replication_protocol'] == 'strong':
      if 'unreplicated' in config['replication_protocol_settings']:
        client_command += (
            ' --strong_unreplicated=%s'
            % str(
                config['replication_protocol_settings']['unreplicated']
            ).lower()
        )
    ...
    if 'message_transport_type' in config['replication_protocol_settings']:
      client_command += (
          ' --trans_protocol %s'
          % config['replication_protocol_settings']['message_transport_type']
      )
    ...
    if 'client_message_timeout' in config:
      client_command += (
          ' --message_timeout %d' % config['client_message_timeout']
      )
    ...
    if config['benchmark_name'] == 'retwis':
      ...
      if config['client_key_selector'] == 'zipf':
        client_command += (
            ' --zipf_coefficient %f' % config['client_zipf_coefficient']
        )

    if (
        'client_wrap_command' in config
        and len(config['client_wrap_command']) > 0
    ):
      client_command = config['client_wrap_command'] % client_command

    if (
        'pin_client_processes' in config
        and isinstance(config['pin_client_processes'], list)
        and len(config['pin_client_processes']) > 0
    ):
      core = config['pin_client_processes'][
          client_id % len(config['pin_client_processes'])
      ]
    ...
    if 'run_locally' in config and config['run_locally']:
      ...
      if (
          'default_remote_shell' in config
        ...
        client_command = tcsh_redirect_output_to_files(
            client_command, stdout_file, stderr_file
        )

    if (
        isinstance(config['client_debug_output'], str)
        or config['client_debug_output']
    ):
      if (
          'run_locally' in config
          and config['run_locally']
          or 'default_remote_shell' in config
          and config['default_remote_shell'] == 'bash'
      ):
  ...
  def get_replica_cmd(
    ...
    if 'run_locally' in config and config['run_locally']:
      ...
      exp_directory = local_exp_directory
      replica_config_path = os.path.join(local_exp_directory, replica_config)
      ...
      exp_directory = remote_exp_directory
      replica_config_path = os.path.join(remote_exp_directory, replica_config)
      shard_config_path = os.path.join(remote_exp_directory, shard_config)
      stats_file = os.path.join(
          exp_directory,
          config['out_directory_name'],
          'server-%d-%d-%d-stats-%d.json'
    ...
    replica_command = ' '.join([
        str(x)
        for x in [
            path_to_server_bin,
            '--network_latency_window',
            config['network_latency_window'],
            '--sent_redundancy',
            config['sent_redundancy'],
            '--loop_queue_interval_us',
            config['loop_queue_interval_us'],
            '--server_id',
            server_id,
            '--replica_config_path',
            replica_config_path,
    ...
    if config['replication_protocol'] == 'strong':
      ...
      if 'max_dep_depth' in config['replication_protocol_settings']:
        replica_command += (
            ' --strong_max_dep_depth %d'
            % config['replication_protocol_settings']['max_dep_depth']
        )
      if 'unreplicated' in config['replication_protocol_settings']:
        replica_command += (
            ' --strong_unreplicated=%s'
    ...
    if config['replication_protocol'] == 'tapir':
      if 'strictly_serializable' in config['replication_protocol_settings']:
        replica_command += (
            ' --tapir_linearizable=%s'
            % str(
                config['replication_protocol_settings']['strictly_serializable']
            ).lower()
        )

    if (
        config['replication_protocol'] == 'morty'
        or config['replication_protocol'] == 'morty-context'
    ):
      if 'branch' in config['replication_protocol_settings']:
        replica_command += (
            ' --morty_branch=%s'
            % str(config['replication_protocol_settings']['branch']).lower()
        )
      if 'prepare_delay_ms' in config['replication_protocol_settings']:
        replica_command += (
            ' --morty_prepare_delay_ms %d'
            % config['replication_protocol_settings']['prepare_delay_ms']
        )

    if (
        config['replication_protocol'] == 'indicus'
        or config['replication_protocol'] == 'pbft'
        or config['replication_protocol'] == 'hotstuff'
    ):
      if 'read_dep' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_read_dep %s'
            % config['replication_protocol_settings']['read_dep']
        )
      if 'watermark_time_delta' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_time_delta %d'
            % config['replication_protocol_settings']['watermark_time_delta']
        )
      if 'sign_messages' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_sign_messages=%s'
            % str(
                config['replication_protocol_settings']['sign_messages']
            ).lower()
        )
        replica_command += (
            ' --indicus_key_path %s'
            % config['replication_protocol_settings']['key_path']
        )
      if 'validate_proofs' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_validate_proofs=%s'
            % str(
                config['replication_protocol_settings']['validate_proofs']
            ).lower()
        )
      if 'hash_digest' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_hash_digest=%s'
            % str(
                config['replication_protocol_settings']['hash_digest']
            ).lower()
        )
      if 'verify_deps' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_verify_deps=%s'
            % str(
                config['replication_protocol_settings']['verify_deps']
            ).lower()
        )
      if 'max_dep_depth' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_max_dep_depth %d'
            % config['replication_protocol_settings']['max_dep_depth']
        )
      if 'signature_type' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_key_type %d'
            % config['replication_protocol_settings']['signature_type']
        )
      if 'sig_batch' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_sig_batch %d'
            % config['replication_protocol_settings']['sig_batch']
        )
      if 'sig_batch_timeout' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_sig_batch_timeout %d'
            % config['replication_protocol_settings']['sig_batch_timeout']
        )
      if 'occ_type' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_occ_type %s'
            % config['replication_protocol_settings']['occ_type']
        )
      if 'read_reply_batch' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_read_reply_batch=%s'
            % str(
                config['replication_protocol_settings']['read_reply_batch']
            ).lower()
        )
      if 'adjust_batch_size' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_adjust_batch_size=%s'
            % str(
                config['replication_protocol_settings']['adjust_batch_size']
            ).lower()
        )
      if 'shared_mem_batch' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_shared_mem_batch=%s'
            % str(
                config['replication_protocol_settings']['shared_mem_batch']
            ).lower()
        )
      if 'shared_mem_verify' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_shared_mem_verify=%s'
            % str(
                config['replication_protocol_settings']['shared_mem_batch']
            ).lower()
        )
      if 'merkle_branch_factor' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_merkle_branch_factor %d'
            % config['replication_protocol_settings']['merkle_branch_factor']
        )
      if 'batch_tout' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_sig_batch_timeout %d'
            % config['replication_protocol_settings']['batch_tout']
        )
      if 'batch_size' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_sig_batch %d'
            % config['replication_protocol_settings']['batch_size']
        )
      if 'ebatch_tout' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_esig_batch_timeout %d'
            % config['replication_protocol_settings']['ebatch_tout']
        )
      if 'ebatch_size' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_esig_batch %d'
            % config['replication_protocol_settings']['ebatch_size']
        )
      if 'use_coord' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_use_coordinator=%s'
            % str(config['replication_protocol_settings']['use_coord']).lower()
        )
      # Added multithreading and batch verification
      if 'multi_threading' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_multi_threading=%s'
            % str(
                config['replication_protocol_settings']['multi_threading']
            ).lower()
        )
      if 'batch_verification' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_batch_verification=%s'
            % str(
                config['replication_protocol_settings']['batch_verification']
            ).lower()
        )
      if 'batch_verification_size' in config['replication_protocol_settings']:
        replica_command += (
            ' --indicus_batch_verification_size %d'
            % config['replication_protocol_settings']['batch_verification_size']
        )
    ...
    if config['benchmark_name'] == 'retwis':
      ...
      if 'server_preload_keys' in config:
        replica_command += (
            ' --preload_keys=%s' % str(config['server_preload_keys']).lower()
        )
      ...
      if 'server_preload_keys' in config:
        ...
        config['benchmark_name'] == 'tpcc'
        or config['benchmark_name'] == 'tpcc-sync'
    ):
      replica_command += ' --data_file_path %s' % config['tpcc_data_file_path']
      replica_command += (
          ' --tpcc_num_warehouses %d' % config['tpcc_num_warehouses']
      )
    elif config['benchmark_name'] == 'smallbank':
      replica_command += (
          ' --data_file_path %s' % config['smallbank_data_file_path']
      )
    ...
    if (
        'server_wrap_command' in config
        and len(config['server_wrap_command']) > 0
    ):
    ...
    if 'run_locally' in config and config['run_locally']:
      ...
      if (
        ...
        replica_command = tcsh_redirect_output_to_files(
            replica_command, stdout_file, stderr_file
        )

    if (
        isinstance(config['server_debug_output'], str)
        or config['server_debug_output']
    ):
      if (
          'run_locally' in config
          and config['run_locally']
  ...
  def prepare_local_exp_directory(self, config, config_file):
    ...
    for instance_idx in range(num_instances):
      ...
      with (
          open(replica_config_path, 'w') as rcf,
          open(shard_config_path, 'w') as scf,
          open(shard_config_path + '_1', 'w') as scf1,
          open(shard_config_path + '_2', 'w') as scf2,
          open(shard_config_path + '_3', 'w') as scf3,
          open(shard_config_path + '_4', 'w') as scf4,
      ):
        ...
        for shard in shards:
          ...
          for replica in shard:
            ...
            print('replica {}:{}'.format(replica, port + 100), file=scf1)
            print('replica {}:{}'.format(replica, port + 200), file=scf2)
            print('replica {}:{}'.format(replica, port + 300), file=scf3)
            print('replica {}:{}'.format(replica, port + 400), file=scf4)
  ...
  def prepare_remote_server_codebase(
      self, config, host, local_exp_directory, remote_out_directory
  ):
    pass
