...
def compile_make(config):
    ...
    if config['client_gc_debug_trace'] or config['server_gc_debug_trace']:
        subprocess.call(["cmake", "-DDEBUG=ON", ".."], cwd=build_path, env=e)
    else:
        subprocess.call(["cmake", "-DDEBUG=OFF", ".."], cwd=build_path, env=e)
